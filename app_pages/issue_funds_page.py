    ...
    if pending_issue_requests.empty:
        ...
        for i, row in pending_issue_requests.iterrows():
            ...
            if st.button(f"Issue Money for {row['Reference ID']}", key=f"issue_{row['Reference ID']}"):
                ...
                # Simulate a refresh by updating session state
                st.session_state["page_refresh"] = st.session_state.get("page_refresh", 0) + 1
                st.experimental_set_query_params(refresh=str(st.session_state["page_refresh"]))
                st.stop()
